...
class App(customtkinter.CTk):
    ...
    def __init__(self, screen_height):
        ...
        self.text_1 = customtkinter.CTkLabel(
            self, text="JSON", font=("Arial", 15), fg_color="transparent"
        )
        self.text_1.grid(row=2, column=0)
        self.text_2 = customtkinter.CTkLabel(
            self, text="CSV", font=("Arial", 15), fg_color="transparent"
        )
        self.text_2.grid(row=2, column=1)

        my_button_1 = customtkinter.CTkButton(
            self, text="<---\n--->", command=self.change_mode, width=38
        )
        my_button_1.grid(row=2, column=0, columnspan=3, pady=(10, 5))

        my_button_2 = customtkinter.CTkButton(
            self, text="CONVERT", command=self.convert, corner_radius=0
        )
    ...
    def convert(self):
        ...
        self.textbox_2.configure(text_color="black")
        ...
        if not text.isspace():
            ...
            if content:
                ...
                self.textbox_2.configure(text_color="red")
                self.textbox_2.insert("0.0", text="ERROR, wrong format")
        else:
            self.textbox_2.configure(text_color="red")
            self.textbox_2.insert("0.0", text="You need to write something")
